...
#define PDPX(v)		PMX((v), 2)
#define PDX(v)		PMX((v), 1)
#define PTX(v)		PMX((v), 0)

#define VMAP		(0xffffffffe0000000ull)
#define VMAPSZ		(256*MiB)

#define KSEG1PML4	(0xffff000000000000ull\
			|(PMX(KSEG1, 3)<<(((3)*PTPGSHFT)+PGSHFT))\
			|(PMX(KSEG1, 3)<<(((2)*PTPGSHFT)+PGSHFT))\
			|(PMX(KSEG1, 3)<<(((1)*PTPGSHFT)+PGSHFT))\
			|(PMX(KSEG1, 3)<<(((0)*PTPGSHFT)+PGSHFT)))

#define KSEG1PTP(va, l)	((0xffff000000000000ull\
			|(KSEG1PML4<<((3-(l))*PTPGSHFT))\
			|(((va) & 0xffffffffffffull)>>(((l)+1)*PTPGSHFT))\
			& ~0xfffull))
...
mmuptpalloc(void)
{
	...
	if((va = mallocalign(PTPGSZ, PTPGSZ, 0, 0)) == nil){
...
pdmap(uintptr pa, int attr, uintptr va, usize size)
{
	...
	for(pae = pa + size; pa < pae; pa += pgsz){
		 ...
		 */
		if(ALIGNED(pa, PSZ(1)) && ALIGNED(va, PSZ(1)) && (pae-pa) >= PSZ(1)){
			assert(*pde == 0);
			*pde = pa|attr|PtePS|PteP;
			...
			if(*pde == 0){
				/*
				 * Need a PTPGSZ physical allocator here.
				 * Because space will never be given back
				 * (see vunmap below), just malloc it so
				 * Ron can prove a point.
				*pde = pmalloc(PTPGSZ)|PteRW|PteP;
				 */
				void *alloc;

				alloc = mallocalign(PTPGSZ, PTPGSZ, 0, 0);
				if(alloc != nil){
...
vmapalloc(usize size)
{
	 ...
	 */
	n = HOWMANY(size, PSZ(0));
	ptsz = PSZ(0)/sizeof(PTE);
	for(i = 0; i < pdsz; i++){
...
mmuinit(void)
{
	 ...
	 */
	pml4 = cr3get();
	sys->pml4[PMX(KSEG1PML4, 3)] = pml4|PteRW|PteP;
