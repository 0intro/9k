 ...
 * and level 0 the PT pages. The PMX macro gives an index into the
 * page-table page at level 'l' for the virtual address 'v'.
 */
#define PG0SZ		(4*KiB)			/* level 0 page size */
#define PG0SHFT		12			/* log(PG0SZ) */

#define PMX(v, l)	(((v)>>(((l)*PTPGSHFT)+PG0SHFT)) & ((1<<PTPGSHFT)-1))
#define PSZ(l)		(1ull<<(((l)*PTPGSHFT)+PG0SHFT))
#define PPF(p, l)	(((l) == 0) ? ((p) & ~((1<<PGSHFT)-1))\
				: ((p) & ~((1<<PTPGSHFT)-1)))	/* wrong */

#define PPN(x)		((x) & ~(PG0SZ-1))
